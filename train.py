...
def train(args):
    ...
    dataset = create_dataset(args.dir)
    batch_ds = dataset.batch(batch_size, drop_remainder=True)

    suffix = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    filename = os.path.join("logdir", suffix, "train")
    summarywriter = tf.summary.create_file_writer(filename)

    stargan = model.Stargan(summarywriter, 128, 40)
    ...
    for _ in range(args.iters):
        ...
        for img, label in dataset.batch(10).take(1):
            ...
            with summarywriter.as_default():
                tf.summary.image(
                    "image", inferred * 0.5 + 0.5, ckpt.step, max_outputs=10
                )

        timetaken = time.time() - initial
        logger.info(f"Speed: {round(timetaken, 5)} epoch/second, {int(ckpt.step)}")
        ckptm.save()
...
def train2(args):
    ...
    cpkt = tf.train.Checkpoint(
        step=tf.Variable(1, dtyle=tf.int32),
        generator=generator,
    ...
    for _ in range(iters):
        inittime = time.time()
        train_step(batched, generator, discriminator, encoder, mapping, summary, ckpt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", help="directory of the celeba", required=True)
    parser.add_argument("--iters", help="number of iterations", default=20)
    parser.add_argument("--bs", help="batch size", default=5)
