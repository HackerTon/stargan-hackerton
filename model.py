...
def one2hot(attr, nlabel, shape=[128, 128]):
    ...
    # cube = tf.TensorArray(tf.float32, nlabel, dynamic_size=True)

    # for i in tf.range(0, 2):
    #     filler = tf.fill(size, attr[i])
    #     cube.write(i, filler)
...
def resi_block(input_layer, k):
    # first block
    d_block = tf.keras.layers.Conv2D(k, (3, 3), (1, 1), padding="same", use_bias=False)(
        input_layer
    )
    ...
    d2_block = tf.keras.layers.Conv2D(
        k, (3, 3), (1, 1), padding="same", use_bias=False
    )(d_block)
...
def generator(nlabel):
    ...
    # c7s1-64
    conv1 = tf.keras.layers.Conv2D(64, (7, 7), 1, padding="same", use_bias=False)(
        combined_input
    )
    ...
    # d128
    conv2 = tf.keras.layers.Conv2D(128, (4, 4), 2, padding="same", use_bias=False)(
        conv1
    )
    ...
    # d256
    conv3 = tf.keras.layers.Conv2D(256, (4, 4), 2, padding="same", use_bias=False)(
        conv2
    )
    ...
    deconv1 = tf.keras.layers.Conv2DTranspose(
        128, (3, 3), 2, padding="same", use_bias=False
    )(res)
    ...
    deconv2 = tf.keras.layers.Conv2DTranspose(
        64, (3, 3), 2, padding="same", use_bias=False
    )(deconv1)
    deconv2 = InstanceNorm()(deconv2)
    deconv2 = tf.keras.layers.ReLU()(deconv2)

    # c7s1-3
    conv4 = tf.keras.layers.Conv2D(3, (7, 7), 1, padding="same", use_bias=False)(
        deconv2
    )
...
def discriminator(width, nlabel):
    ...
    src_output = tf.keras.layers.Conv2D(
        1, (3, 3), (1, 1), padding="same", use_bias=False, name="src"
    )(output)
...
class Stargan:
    def __init__(
        self, summary, width, nlabel, lambda_cls=1, lambda_rec=10, lambda_gp=10
    ):
